...
class OysterJourney {
    private $_username = OYSTER_USERNAME; // your oyster tfl username
    private $_password = OYSTER_PASSWORD; // your oyster tfl password
    private $_cookie_file = COOKIE_FILE; // absolute path to a text file to be used to save the cookie in
    private $_success = false;
    private $_order_journey = 'desc';
    private $_order_all = 'desc';
    /**
     * Do some lovely validation so that we can check to make sure everything is
     * provided, then attempt the login and grab the cookie
     * 
     * @throws Exception 
     */
        ...
        if(!empty($username) && !empty($password)) {
            ...
            if(!empty($cookie_file) && is_file($cookie_file)) {
                if(is_writable($cookie_file)) {
                    ...
                    throw new Exception('the cookie is not writeable');
                }
            } else {
                throw new Exception('Cookie file doesn\'t exist '. __DIR__ . '/' . $this->_cookie_file);
            }
        } else {
            throw new Exception('Missing parameter(s) \'user\' and \'password\' ');
        }
    }
    /**
    ...
    private function loginAndGetCookie($username, $password, $cookie_file) {
        ...
        if(preg_match("/Login failed\. Please check your username and password and try again\./", $return, $matches) !== 0) {
            throw new Exception('Wrong credetials');
        } else {
            $this->_success = true;
        }
    }
